...
fun errLocation (input_name, input_stream, input_lexbuf) loc =
  ...
  if (getLexStartPos input_lexbuf,
      getLexLastPos input_lexbuf) = (0,0) then
    let
      val curr_pos = ref 0
      fun input () =
        let val buf = getLexBuffer input_lexbuf
            val c =
          if !curr_pos >= String.size buf - 1 then
            raise Size
          else if !curr_pos >= 0 then
            (CharVector.sub(buf, !curr_pos)
             handle Subscript => #"\n")
          else
            #"."
    ...
    in
      errPrompt "Meta.exec Toplevel input:";
