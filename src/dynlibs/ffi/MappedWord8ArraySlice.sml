(* MappedWord8ArraySlice -- SML Basis Library 
   sestoft@dina.kvl.dk 2000-10-24
   Modified for Mapped buffers by IG 2014-12-10
*)

(* Word8ArraySlice -- SML Basis Library *)

structure MappedWord8ArraySlice :> MappedWord8ArraySlice =
struct
    prim_type array_
    type elem = Word8.word
    type array = MappedWord8Array.array
    type vector = Word8Vector.vector
    type vector_slice = Word8VectorSlice.slice
 local
    prim_val magic   : 'a -> 'b                        = 1 "identity"

    fun from_array (a : array)  = magic (MappedWord8Array.get_cptr a) : array_
    fun length_ (a : array) = MappedWord8Array.length a
    val to_cptr : array_ -> Dynlib.cptr = magic
...
fun vector (a : array, i, n) = 
	...
	fun copy j = 
	    if j<n then
		(updatev_ newvec j (sub_ a (i+j)); copy (j+1))
	    else
		()
