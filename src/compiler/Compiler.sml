...
(* Lexer of string *)
fun createLexerString s = Lexing.createLexerString s
; 
...
val parseTopDecPhrase =
  parsePhraseAndClear Parser.TopDecFile Lexer.Token
;
...
fun elabToplevelDecPhrase dec =
  let (* val _ = Asyntfn.printDec dec (* cvr: *) *)
      val (iBas,resdec) = resolveToplevelDec dec
      val ExEnv = 
	  let val ExEnv = elabToplevelDec resdec
          in
              resolveOvlDec resdec;
              commit_free_typevar_names ();
              ExEnv
          end
	  handle e => (rollback_free_typevar_names ();
		       raise e) 
      fun updateCurrentCompState ((iBas, ExEnv as EXISTS(T,(ME,FE,GE,VE, TE)))) =
          (updateCurrentInfixBasis iBas;
           updateCurrentStaticT T;
           updateCurrentStaticME ME;
           updateCurrentStaticFE FE;
  ...
  in 
     updateCurrentCompState (iBas, ExEnv);
     (iBas, ExEnv)
  end 
;
