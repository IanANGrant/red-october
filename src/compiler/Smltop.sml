...
fun loadTopDecPhrase lexbuf =
  let val str = parseTopDecPhrase lexbuf in
    execTopDecPhrase str
  end
;

fun loadToplevelPhraseString mode s =
   let open Mixture
       val phr = Compiler.createLexerString s
   in 
        (protect_current_input
           (fn () =>
               (input_name := "";
                input_stream := std_in;
                input_lexbuf := phr;
                if mode = STRmode then
                   while true do
                      let val isLast = loadToplevelPhrase phr
                      in if isLast then raise EndOfFile else ()
                      end
                else loadTopDecPhrase phr)));
         true
   end
      handle
          EndOfFile =>
...
[
  ...
  ("exec",   { qualid={qual="Meta", id=["exec"]},      info=VARname REGULARo}),
  ("execToplevel",   { qualid={qual="Meta", id=["execToplevel"]},      info=VARname REGULARo}),
  ...
  ("metaquotation",
             { qualid={qual="Meta", id=["metaquotation"]}, info=VARname REGULARo}),
  ("utf8",
...
[
   ...
   ("exec",        trivial_scheme(type_arrow type_string type_bool)),
   ("execToplevel",        trivial_scheme(type_arrow type_string type_bool)),
   ...
   ("metaquotation",trivial_scheme(type_ref type_bool)),
...
fun resetSMLTopDynEnv() =
  loadGlobalDynEnv "Meta" [
    ...
    ("exec",        repr (loadToplevelPhraseString STRmode)),
    ("execToplevel",        repr (loadToplevelPhraseString TOPDECmode)),
    ...
    ("metaquotation",repr Lexer.metaquotation),
